...
from View.Controller.dx_t_img import convert_dtimg, convert_one_img
...
def nameSection(master, frame_width, frame_height, root):
    ...
    print(f'Name Section Width: {section_width}')
    print(f'Name Section Height: {frame_height}\n')
...
def register_filter(input_text, selection, quarter, errorLabel, edit_mode=False, old_name=None, new_name=None):
    ...
    if not all(word.replace('.','').isalpha() for word in input_text.split()): 
        ...
        errorLabel.pack(side=ctk.LEFT, expand = True, fill=ctk.BOTH)
        return print('Special Characters are not allowed')
    ...
    for n in name:
        if input_text in n[0] and not edit_mode:
            ...
            errorLabel.pack(side=ctk.LEFT, expand = True, fill=ctk.BOTH)
            return print(f'{n[0]} is already registered')
    ...
    if not selection: 
        ...
        errorLabel.pack(side=ctk.LEFT, expand = True, fill=ctk.BOTH)
        return print("Please add a Selection Option")

    # Empty Quarter
    if not quarter: 
        errorLabel.configure(text='Please add a Quarter Option')
        errorLabel.pack(side=ctk.LEFT, expand = True, fill=ctk.BOTH)
        return print("Please add a Quarter Option")
...
def sideBar(master, frame_width, frame_height):
    ...
    print(f'Side Bar Width: {int(frame_width * 0.35)}')
    print(f'Side Bar Height: {frame_height}\n')
...
def toggle_loading_animation():
    ...
    if loading_label.winfo_ismapped():
        print(f'Removing Loading Animation\n')
        loading_frame.pack_forget()
    else:
        print(f'Loading Animation Initializing\n')
...
def toggle_template_sidebar():
    ...
    # Print the current width and height of the window
    current_width = root.winfo_width()
    current_height = root.winfo_height()
    print(f"Current window size - Width: {current_width}, Height: {current_height}")
...
def filter_pageProcedure(file_label):
    ...
    if file_label.cget("text") == "No file selected":
        ...
        print(f'Fail ( Add a template prompt )')
        return
    ...
    if one_click_preview:
        ...
        return print('Please wait for the current process to finish')
