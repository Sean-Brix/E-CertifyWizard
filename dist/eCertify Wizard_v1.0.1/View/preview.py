...
def openPreview(master):
    ...
    print(f'Preview Page Rendering: Sucessful\n')
...
def sidebarSection():
    ...
    print(f'Preview Sidebar Width: {int(frame_width * 0.05)}')
    print(f'Preview Sidebar Height: {frame_height}\n')
...
def editingSection():
    ...
    print(f'Editing Section Width: {frame_width}')
    print(f'Editing Section Height: {int(frame_height * 0.6)}\n')
...
def kvpair_edit_mode():
    ...
    for pair in kvPair_widget:
        ...
        if not key or not value:
            ...
            return print('Cannot save empty fields: kvpair_edit_mode()')
...
def kvpair_save_edit():
    ...
    print(f'Disabled: {disable_list}')
    print(f'Key Pre Determine: {key_pre_determine}')
...
def clear_all_kvPairs():
    ...
    if messagebox.askokcancel("Confirm", "Are you sure you want to clear all pairs?"):
...
def format_setting_section(master):
    global fileLabel_format, output_types, image_checkbox_var, pdf_checkbox_var, docx_checkbox_var
    ...
    Format_settings_label.pack(side=ctk.TOP, fill=ctk.BOTH, expand=True, padx=10, pady=10)

    format_settings_frame = ctk.CTkFrame(
        format_output_section, 
        fg_color="transparent"
    )
    format_settings_frame.pack(side=ctk.TOP, fill=ctk.BOTH, expand=True, padx=10, pady=10)
    ...
    image_checkbox.pack(side=ctk.TOP, anchor='w', padx=5, pady=5)
    ...
    pdf_checkbox.pack(side=ctk.TOP, anchor='w', padx=5, pady=5)
    ...
    docx_checkbox.pack(side=ctk.TOP, anchor='w', padx=5, pady=5)

    # Directory input
    file_dialog_frame = ctk.CTkFrame(
        format_output_section, 
        fg_color="transparent"
    )
    file_dialog_frame.pack(side=ctk.BOTTOM, fill=ctk.BOTH, expand=True, padx=10, pady=10)
    ...
    file_label.pack(side=ctk.TOP, pady=5, padx=25, expand=True, fill=ctk.X)
    fileLabel_format = file_label

    file_dialog_button = ctk.CTkButton(
        file_dialog_frame, 
        text="Select Output Directory", 
        command=lambda: open_file_dialog()
    )
    file_dialog_button.pack(side=ctk.TOP, anchor='w', padx=5, pady=5)
    ...
    publish_btn.pack(side=ctk.TOP, padx = 10, pady = 10)
...
def update_output_types():
    global output_types, image_checkbox_var, pdf_checkbox_var, docx_checkbox_var
...
def open_file_dialog():
    ...
    print(f'Asking for directory: ', end ='')

    output_directory = filedialog.askdirectory()
    filename = basename(output_directory)

    if not filename:
        print(f'Failed ( No File Selected )\n')
        glo_errorLabel.configure(text="No folder selected")
        return
    ...
    if len(filename) <= 18:
        fileLabel_format.configure(text=f"Selected Folder: {filename}")
    else:
        fileLabel_format.configure(text=f"Selected Folder: {filename[:18]} . . .")

# COPY THE FILES
def add_to_output_directory():
    global output_directory, reg_names, inputFile_name
...
def prev_section():
    ...
    print(f'Preview Image Frame: {frame_width}')
    print(f'Editing Section Height: {prvFram_height}\n')
...
def change_preview_image():
    ...
    if new_img is None:
        print('Failed ( Image conversion failed )\n')
...
def goBack_regPage():
    global kvPair_widget, kvpairLabels, kvedit_mode, back_page, add_on
